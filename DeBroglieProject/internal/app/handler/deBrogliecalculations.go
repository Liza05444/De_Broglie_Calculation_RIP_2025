...
func (h *Handler) UpdateCalculationDeBroglieLengthAPI(ctx *gin.Context) {
	...
	isSuccess := req.Success != nil && *req.Success
	if !isSuccess || req.DeBroglieLength == nil {
		...
		ctx.JSON(http.StatusOK, gin.H{
			"message": "заявка на расчет длины волны де Бройля обработана (неудача)",
		})
		return
	}
	...
	} else if request.Status == ds.RequestStatusFormed && request.FormedAt != nil {
		timeSinceFormed := time.Since(*request.FormedAt)
		if timeSinceFormed > 15*time.Second {
			err = h.Repository.UpdateDeBroglieRequestStatus(requestID, ds.RequestStatusRejected, nil)
			if err != nil {
				h.errorHandler(ctx, http.StatusInternalServerError, err)
				return
	...
	ctx.JSON(http.StatusOK, gin.H{
		"message": "заявка на расчет длины волны де Бройля обработана",
	})
}
